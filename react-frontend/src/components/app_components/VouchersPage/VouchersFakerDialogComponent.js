...
import vouchersFakerFactory from "./vouchersFakerFactory";
...
const VouchersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = vouchersFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("vouchers").create(elem));
...
export default connect(mapState, mapDispatch)(VouchersFakerDialogComponent);
